...
_Annalise Cramer contributed to this exercise._
...
I added two variables. The first variable is BMI, I calculated BMI using height and weight wherever available, left blank if i could not calculate. The second is the categorical variable on marital status which can be coded as married, single, divorced. 
...
@tbl-resulttable3 shows a summary of a linear model fit.
...
```
_The results from the linear regression model shows that for every unit increase in the BMI the height increases by 2.14 cm controlling to marital status (p-value: 0.011). Similarly being single was associated with a decrease of 37.37 cm in height compared to the divorced(p-value: 0.010). However being married was associated with an increase of 10.56 cm in height compared to divorced which was not statistically significant(p-value: 0.10)._ 
...
```{r}
#| label: fig-result2
#| fig-cap: "Height by marital status."
#| echo: FALSE
knitr::include_graphics(here("starter-analysis-exercise","results","figures","height-by-marital-status.png"))
```
_The boxplot shows the distribution of height across different marital status groups.Individuals who are married have highest median height with less variability. However, individual who are single are having less median height._  

@fig-result3 shows a scatter plot produced by one of the R scripts.
```{r}
#| label: fig-result3
#| fig-cap: " Scatter plot weight and bmi."
#| echo: FALSE
knitr::include_graphics(here("starter-analysis-exercise","results","figures","weight-and-bmi.png"))
```
_The scatter plot shows a relationship between BMI and weight. As BMI increases, weight also tends to increase._
